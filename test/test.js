...
describe('DataStore', () => {
  ...
  describe('fetch()', () => {
    ...
    it('should resolve with stale value', () => {
      store.set('foo/__expiry', { expires: Infinity, expiresIfError: Infinity });
      return store.fetch('foo', 'foo', {}).then(response => {
    ...
    it('should resolve with stale value when failure loading and "options.rejectOnError=false"', () => {
      ...
      return store.fetch('foo', 'http://localhost/foo', { rejectOnError: false }).then(response => {
        ...
        expect(Number(new Date(response.headers.expires)) - Date.now())
          .to.be.greaterThan(59000)
          .lessThan(61000);
