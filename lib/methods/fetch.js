...
function doFetch(store, key, url, options) {
  ...
  if (isMissingOrExpired) {
    ...
    // Prevent unhandled
    promiseToLoad.catch(function (err) {
...
function load(store, key, url, options) {
  ...
  return agent.get(url, options).timeout(timeout).retry(retry).then(function (res) {
...
function getExpiry(dateString, minimum, grace) {
  ...
  var expires = Number(new Date(dateString)) + grace;
  var now = Date.now();

  return expires > now ? expires : // Local clock is set incorrectly
  now + minimum;
}
